...
每日新增 `gameislearning` 收集器，依臺北、新北、桃園篩選 [台語站活動頁](https://www.gameislearning.url.tw/taigi.php)，再讀每個活動詳情。使用者指定頁內活動皆為台語活動，因此語言不送人工審查；日期、開始時間、地點、是否過期與跨來源重複仍逐場檢查。公告內只有純文字的 BeClass、Google Forms、ACCUPASS、OPENTIX、Linktree 等 HTTPS 網址也會抽出，正式網站詳情分開提供可點的「報名／買票」與「活動公告」。
