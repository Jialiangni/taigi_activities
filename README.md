...
目前自動核實支援 OPENTIX 的單場 API 與官方 HTML 交叉檢查、ACCUPASS 明列「台語場」且完整的單一場次，以及使用者指定語言可信的台語站。台語站不再判斷語言，但仍要求明確場次、北北桃地點並檢查過期與重複。ACCUPASS 搜尋本來就以每個台語關鍵字搭配臺北／新北／桃園三個城市代碼執行；候選沒刊登不等於搜尋沒找到。字幕、人物台語經歷、泛關鍵字、場次總期間不作為其他來源的自動通過證據；系列或資訊不完整者仍待核實。自動核實與人工核實在 verification.mode／method 區分，保留原有人工紀錄。
