...
目前自動核實支援 OPENTIX 的單場 API 與官方 HTML 交叉檢查，以及 ACCUPASS 明列「台語場」、完整單場時間、北北桃地點、主辦、正常狀態與免費報名的單一場次。ACCUPASS 搜尋本來就以每個台語關鍵字搭配臺北／新北／桃園三個城市代碼執行；候選沒刊登不等於搜尋沒找到。字幕、人物台語經歷、泛關鍵字、場次總期間不作為自動通過證據；系列或資訊不完整者仍待核實。其他來源仍會檢查重複並列待核實，沒有宣稱全部候選已完成人工判讀。自動核實與人工核實在 verification.mode／method 區分，保留原有人工紀錄。
