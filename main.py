...
def build(output_dir=Path('.'), check_sources=False, write_ai=False):
    if write_ai and not check_sources:
        raise ValueError('AI 編輯前必須通過 --check-sources')
    ...
    if write_ai:
        from crawler.ai_editorial import enrich
        enrich(activities)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--write-ai', action='store_true', help='來源重查通過後，僅替新增活動編寫台文')
    args = parser.parse_args()
    build(args.output_dir, args.check_sources, args.write_ai)
