...
首次匯入指引：網站按鈕改為「Apple 日曆：先看預覽」，緊接可見且以 aria-describedby 關聯的兩步驟提示。單場 ICS 的 DESCRIPTION 開頭加入同一提示：第一步是預覽，右上角 ✓ 僅關閉；下方 Add to Calendar 進入第二步，最後 Add／加入才完成。系統按鈕文字及預覽畫面本身無法由網站修改；提示會留在匯入後的活動備註，介面亦明示。原活動描述完整保留在提示後方，UID、標題、日期與地點不變；全站／篩選 ICS 不附此提示。日曆連結加上 preview=1 以區分先前無提示的檔案快取。iOS 如何呈現 DESCRIPTION 仍由系統決定，不能宣稱提示一定出現在原生預覽頂端。
