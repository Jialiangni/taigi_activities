...
手機顯示「佗位」與「幾月」標籤，兩列標籤欄皆30px，第一個「攏總」按鈕皆52px且左右對齊。城市及月份按鈕依aria-pressed呈現選取反白：深色模式為淺底深字、淺色模式為深底淺字；選中的城市按鈕取消內層彩色底以保持整顆反白，未選城市及活動卡片仍保留城市底色。城市單選、月份複選及「攏總」重設行為維持不變。
...
臺北、新北、桃園以藍、綠、橘的圓角底色區分，取代原有小圓點，套用於城市篩選、活動卡片、清單、週曆圖例及詳情。深淺模式各有對應底色，保留各位置原有文字顏色、字級及字重。城市篩選的選取狀態仍使用既有前景色，標籤底色配合選取背景調整；不改變活動資料與篩選行為。
