...
from .editorial import session_editions, session_edition
...
def load_verified(path=DATA_PATH, now=None, check_sources=False):
    ...
    editorial_entries = session_editions() if Path(path).resolve() == DATA_PATH.resolve() else []
    for row in payload['activities']:
        edition = session_edition(row['activity'], editorial_entries, strict=True)
        if edition:
            ...
            if not edition['source_quotes'] or any(q not in required for q in edition['source_quotes']):
                raise ValueError('活動介紹缺少來源重查證據：' + row['activity']['id'])
